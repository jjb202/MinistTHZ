...
class Session:
    def __init__(self):
        ...
        scheduler_cosine = optim.lr_scheduler.CosineAnnealingLR(self.g_opt, 37500,
                                                            eta_min=1e-6)
        self.g_sche = GradualWarmupScheduler(self.g_opt, multiplier=1, total_epoch=3,
                                       after_scheduler=scheduler_cosine)

    def tensorboard(self, name):
        self.writers[name] = SummaryWriter(os.path.join(self.log_dir, name + '.events'), comment='Linear')
    ...
    def train(self, name, batch):
        ...
        torch.nn.utils.clip_grad_norm(self.net.parameters(), 0.01)
        self.g_opt.step()
...
def run_train(ckp_name='latest', folder='mynet'):
    ...
    while sess.step <= sess.step_num:
        ...
        # lo.append(losses)
        # loss0 = np.array(lo)
        # np.save('./lossPlot/epoch_{}'.format(sess.step),loss0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--folder', default='pmann')  # 可以改默认配置，把模型保存在不同的文件夹下
