...
##########################################################################
## Channel Attention Block (CAB) #输入输出分辨率不变
class CAB(nn.Module):
...
class MPRNet(nn.Module):
    def __init__(self, in_c=1, out_c=1, n_feat=16, scale_unetfeats=16, scale_orsnetfeats=16, num_cab=2, kernel_size=3, reduction=4, bias=False):
        ...
        self.sam12 = SAM(n_feat, kernel_size=1, bias=bias) # 输出分辨率不变
        self.sam23 = SAM(n_feat, kernel_size=1, bias=bias)
        
        self.concat12  = conv(n_feat*2, n_feat, kernel_size, bias=bias)
        self.concat23  = conv(n_feat*2, n_feat+scale_orsnetfeats, kernel_size, bias=bias)
        self.tail     = conv(n_feat+scale_orsnetfeats, out_c, kernel_size, bias=bias)
        self.up = nn.Upsample((256,256))
        self.up1 = nn.Upsample((28,28))
        self.convo = nn.Conv2d(3,1,3,1,1)
    def forward(self, x3_img):
        # Original-resolution Image for Stage 3
        x3_img = self.up(x3_img)
        H = x3_img.size(2)
        W = x3_img.size(3)
        ...
        # Four Patches for Stage 1  # 图像拆分成4分
        ...
        x1ltop = self.shallow_feat1(x1ltop_img)  # 14*14
        x1rtop = self.shallow_feat1(x1rtop_img)
        x1lbot = self.shallow_feat1(x1lbot_img)
        x1rbot = self.shallow_feat1(x1rbot_img)
        # print("x1ltop.size{}".format(x1ltop.size()))
        
        ## Process features of all 4 patches with Encoder of Stage 1
        feat1_ltop = self.stage1_encoder(x1ltop)  # 7/4=1*1
        feat1_rtop = self.stage1_encoder(x1rtop)
        feat1_lbot = self.stage1_encoder(x1lbot)
        feat1_rbot = self.stage1_encoder(x1rbot)
        # print("feat1_ltop.size{}".format(feat1_ltop.size()))
        
        ## Concat deep features
        feat1_top = [torch.cat((k,v), 3) for k,v in zip(feat1_ltop,feat1_rtop)]
        feat1_bot = [torch.cat((k,v), 3) for k,v in zip(feat1_lbot,feat1_rbot)]
        # print("feat1_top.size{}".format(feat1_top.size()))
        
        ## Pass features through Decoder of Stage 1
        res1_top = self.stage1_decoder(feat1_top)
        res1_bot = self.stage1_decoder(feat1_bot)
        # print("res1_top.size{}".format(res1_top.size()))
        ...
        x3_img = self.up1(x3_img)
        stage3_img = self.up1(stage3_img)
        stage2_img = self.up1(stage2_img)
        stage2_img = self.convo(stage2_img)
        stage1_img = self.up1(stage1_img)
        stage1_img = self.convo(stage1_img)
