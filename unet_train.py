...
class Session:
    def __init__(self):
        ...
        if torch.cuda.is_available():
            self.net = UnetGenerator(1,1).cuda()
            self.crit = nn.L1Loss().cuda()
            self.ssim = SSIM().cuda()
        else:
            self.net =UnetGenerator(1,1)
            self.crit = nn.L1Loss()
            self.ssim = SSIM()
        ...
        self.writers = {}
        # 优化器选择Adam
        self.g_opt = Adam(self.net.parameters(), lr=settings.lr)
    ...
    def train(self, name, batch):
        ...
        if torch.cuda.is_available():
            ...
            label= label.cuda()
        pred = self.net(image)
        loss_list = [self.crit(pred, label)]
        losses = {
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--folder', default='unet')  # 可以改默认配置，把模型保存在不同的文件夹下
